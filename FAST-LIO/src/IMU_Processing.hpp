...
class ImuProcess
{
 public:
  ...
  void Process(const MeasureGroup &meas,  
                        ...
                        esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_point);
  ofstream fout_imu;    
  V3D cov_acc;                
  V3D cov_gyr;                
  V3D cov_acc_scale;   
 ...
 private:
  void IMU_init(const MeasureGroup &meas, 
                        esekfom::esekf<state_ikfom, 12, input_ikfom> 
                        &kf_state, int &N);
  void IMU_init_dynamic(const MeasureGroup &meas, 
                        esekfom::esekf<state_ikfom, 12, input_ikfom> 
                        &kf_state, int &N, esekfom::esekf<state_ikfom, 12, input_ikfom> 
                        &kf_point);
...
void ImuProcess::Process(const MeasureGroup &meas,  
                                                    esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, 
                                                    PointCloudXYZI::Ptr cur_pcl_un_, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_point)
{
  double t1,t2,t3;
  t1 = omp_get_wtime();
  ...
  if (imu_need_init_)
  {
    if(imu_init_dynamic)
    {
      IMU_init_dynamic(meas, kf_state, init_iter_num, kf_point);
    }else{
